...
public final class AKMediaMetadataProvider: AKMediaMetadataProviderProtocol, @unchecked Sendable {
    ...
    public func updateStaticMetadata(_ metadata: AKMediaStaticMetadata) {
        let merged = state.withLock { s -> AKMediaStaticMetadata in
            ...
            if let v = metadata.albumTitle {
                updated.albumTitle = v
            }
            if let v = metadata.albumArtist {
                updated.albumArtist = v
            }
            if let v = metadata.genre {
                updated.genre = v
            }
            if let v = metadata.composer {
                updated.composer = v
            }
            ...
            if let v = metadata.trackCount {
                updated.trackCount = v
            }
            if let v = metadata.discNumber {
                updated.discNumber = v
            }
            if let v = metadata.discCount {
                updated.discCount = v
            }
            if let v = metadata.releaseDate {
                updated.releaseDate = v
            }
            if let v = metadata.creationDate {
                updated.creationDate = v
            }
            if let v = metadata.descriptionText {
                updated.descriptionText = v
            }
            if let v = metadata.copyrights {
                updated.copyrights = v
            }
            if let v = metadata.publisher {
                updated.publisher = v
            }
    ...
    public func handleTimedMetadata(_ items: [AVMetadataItem]) {
        ...
        Task { [weak self] in
            ...
            for item in items {
                ...
                } else if item.commonKey == .commonKeyArtist || item
    ...
    public func loadMetadata() async {
        ...
        let task = Task { [weak self] in
            guard let self else { return }
            await loadCommonMetadata(from: asset)
            await loadFormatSpecificMetadata(from: asset)
        }
    ...
    private func parseID3Item(
        _ item: AVMetadataItem,
        keyString: String?,
        into meta: inout AKMediaStaticMetadata
    ) async {
    ...
    private func parseITunesItem(
        ...
        case "trkn" where meta.trackNumber == nil:
            if let data = try? await item.load(.dataValue), data.count >= 6 {
                // Bytes 2-3 are track number, bytes 4-5 are track count (big-endian 16-bit
                // integers)
    ...
    private func parseQuickTimeItem(
        _ item: AVMetadataItem,
        keyString: String?,
        into meta: inout AKMediaStaticMetadata
    ) async {
