...
public protocol AKMediaManagerProtocol: AnyObject, Sendable {
    ...
    /// Evaluates whether seeking to a target time is permitted and returns an unavailability reason
    /// if disallowed.
    /// - Parameter target: The target seek target position.
    /// - Returns: A tuple containing a boolean flag indicating permission and an optional
