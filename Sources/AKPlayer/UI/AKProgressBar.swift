...
//
    ...
    public struct AKProgressBarConfiguration: Sendable, Equatable {
        ...
        ///   - snapThresholdFraction: The fractional scrubbing distance threshold for magnetic
        /// snapping.
        ///   - enableSnapping: Whether scrubbing automatically snaps to nearby ad cue markers.
        ///   - enforceRestrictions: Whether forward scrubbing restrictions past unplayed
        /// interstitials are enforced.
        public init(
    ...
    public struct AKProgressBar: View {
        ...
        @State private var isRestrictedAtMarker = false
        ...
        /// The body view rendering the interactive progress bar track, buffer, ad markers,
        /// chapters, and thumb.
        public var body: some View {
            GeometryReader { geometry in
                ...
                let currentHeight = isDragging ? configuration.activeTrackHeight : configuration
                    .trackHeight
                ...
                ZStack(alignment: .leading) {
                    ...
                    if bufferProgress > 0 {
                        Capsule()
                            ...
                            .frame(
                                width: max(0, width * CGFloat(bufferProgress)),
                    ...
                    Capsule()
                        ...
                        .frame(
                            width: max(0, width * CGFloat(effectiveFraction)),
                            height: currentHeight
                        )
        ...
        private func adFillRangesView(width: CGFloat, height: CGFloat) -> some View {
            if duration > 0 {
                ForEach(markers.filter(\.isFill)) { marker in
                    let startFraction = max(0, min(1, marker.time / duration))
                    let segmentDuration = marker.duration ?? 0
                    let endFraction = max(
                        startFraction,
                        min(1, (marker.time + segmentDuration) / duration)
                    )
                    let segmentWidth = max(2, width * CGFloat(endFraction - startFraction))
                    let offset = width * CGFloat(startFraction)

                    let fillColor: Color = if marker.isCurrent {
                        configuration.activeMarkerColor.opacity(0.85)
                    } else if marker.isPlayed {
                        configuration.playedMarkerColor.opacity(0.4)
                    } else {
                        configuration.fillMarkerColor
                    }
        ...
        private func adCueMarkersView(width: CGFloat) -> some View {
            if duration > 0 {
                ForEach(markers.filter(\.isSinglePoint)) { marker in
                    let markerFraction = max(0, min(1, marker.time / duration))
                    let xPosition = width * CGFloat(markerFraction)
                    let markerColor: Color = if marker.isCurrent {
                        configuration.activeMarkerColor
                    } else if marker.isPlayed {
                        configuration.playedMarkerColor
                    } else {
                        configuration.unplayedMarkerColor
                    }
                    ...
                    ZStack {
                        Circle()
                            ...
                            .frame(
                                width: marker.isCurrent ? configuration
                                    .markerDiameter + 2 : configuration.markerDiameter,
                                height: marker.isCurrent ? configuration
                                    .markerDiameter + 2 : configuration.markerDiameter
                            )
                            .shadow(
        ...
        /// Generates the circular scrubber thumb positioned at the current or dragged playback
        /// fraction.
        /// - Parameters:
        ///   - width: The total pixel width of the progress bar track.
        ...
        private func scrubberThumbView(width: CGFloat, fraction: Double) -> some View {
            ...
            Circle()
                ...
                .frame(
                    width: isDragging ? configuration.thumbDiameter + 4 : configuration
                        .thumbDiameter,
                    height: isDragging ? configuration.thumbDiameter + 4 : configuration
                        .thumbDiameter
                )
