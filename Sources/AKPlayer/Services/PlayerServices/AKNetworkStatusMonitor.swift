...
/// A thread-safe monitor class responsible for tracking network connectivity changes using
/// `NWPathMonitor`
...
public final class AKNetworkStatusMonitor: AKNetworkStatusMonitorProtocol, Sendable {
    ...
    /// The current network connectivity status (`.satisfied`, `.unsatisfied`, or
    /// `.requiresConnection`).
    ...
    public func startObserving() {
        ...
        monitor.pathUpdateHandler = { [weak self] path in
            ...
            state.withLock { $0.latestPath = path }
            eventBroadcaster.send(path.status)
        }
