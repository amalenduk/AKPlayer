...
/// Low-level coordinator wrapping system `MPNowPlayingSession`, `MPRemoteCommandCenter`, and `MPNowPlayingInfoCenter`.
@MainActor
public final class AKNowPlayingSession: AKNowPlayingSessionProtocol {
    ...
    /// Registers target-action handlers on the MPRemoteCommandCenter for the specified command if not already added.
    ...
    /// Invokes the registered handler for the given remote command event, returning the handler status.
    /// - Parameters:
