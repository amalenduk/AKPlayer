...
public protocol AKSeekingThroughMediaServiceProtocol: AnyObject, Sendable {
    ...
    /// Evaluates whether a given target is within seekable bounds and provides an explicit failure
    /// reason if blocked.
    /// - Parameter target: The target seek position.
...
/// A stateless verification service providing boundary validation for seek actions on an
/// `AVPlayerItem`.
    ...
    @unchecked Sendable
{
    ...
    /// - Parameter mediaManager: The parent media manager instance providing access to the player
    /// item.
        ...
        -> (flag: Bool, reason: AKPlayerUnavailableCommandReason?)
    {
        ...
        if case .live = target {
            ...
            return !ranges.isEmpty ? (true, nil) : (playerItem.duration.isNumeric ? (true, nil) : (
                false,
